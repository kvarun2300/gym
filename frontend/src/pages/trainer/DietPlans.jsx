...
const DietPlans = () => {
  ...
  useEffect(() => setTitle('Diet Plans'), [setTitle]);

  const fetchPlans = useCallback((page = 1) => {
    setLoading(true);
    trainerService
      .getDietPlans({ page, limit: 10 })
      .then(({ data }) => {
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try {
      await trainerService.deleteDietPlan(deleteTarget.id);
      ...
      render: (row) =>
        row.targetCalories ? (
          <span className="flex items-center gap-1 text-xs">
            <Flame size={12} className="text-crimson-light" /> {row.targetCalories} kcal
          </span>
        ) : (
          '—'
        ),
    },
    { key: 'meals', label: 'Meals', render: (row) => `${row.meals?.length || 0} meals` },
    {
      key: 'status',
      label: 'Status',
  ...
  return (
    <div className="space-y-5">
      ...
      <DataTable columns={columns} rows={rows} loading={loading} emptyLabel="No diet plans created yet" />
